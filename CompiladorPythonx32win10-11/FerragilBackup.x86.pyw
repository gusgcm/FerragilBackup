...
class FileCopierApp:
    def __init__(self, master):
        # Definir prioridade de I/O baixa
        ...
        for attempt in range(retries):
            try:
                ...
                with open(src, 'rb') as fsrc, open(dst, 'wb') as fdst:
                    shutil.copyfileobj(fsrc, fdst, length=128*1024*1024)  # Buffer de 128MB
    ...
    def copiar_arquivos(self, origem, destino, item, progress_callback, start_time):
        ...
        total_files = 0
        update_counter = 0
        file_metadata = {}  # Cache de metadados
    ...
    def start_copy_all(self):
        ...
        if not self.directory_pairs:
            messagebox.showwarning("Nenhum Diretório", "Nenhum diretório configurado.")
            return
    ...
    def copiar_todos(self, show_message=True):
        ...
        def update_progress(copied, total, item, start_time):
            if self.master.state() == "normal":  # Atualizar GUI apenas se visível
    ...
    def run_backup_with_retries(self, show_message=False, max_retries=3, retry_delay=300):
        ...
        while retries < max_retries and not self.stop_automation.is_set():
            self.copying = True
            self.copiar_todos(show_message)
            self.copying = False
            if total_files_copied > 0:
                break
            retries += 1
        ...
        while not self.stop_automation.is_set():
            ...
            if self.automation_var.get() == 1:
                ...
                for schedule in self.config_data.get("schedules", []):
                    ...
                    elif (now.hour == schedule["hour"] and
                          now.minute == schedule["minute"] and
                          (schedule["hour"], schedule["minute"], current_date) not in triggered_schedules and
                          not self.copying and
                          (not last_backup_time or last_backup_time.date() < now.date())):
                        self.run_backup_with_retries(show_message=False)
                ...
                if next_schedule:
                    ...
                    time.sleep(min(seconds_to_next, 3600))  # Dormir até o próximo agendamento ou 1 hora
                else:
                    time.sleep(60)  # Intervalo padrão de 60 segundos
            else:
                time.sleep(60)
    ...
    def set_autostart(self, enable):
        ...
        try:
            ...
            self.master.after(0, lambda: messagebox.showerror("Erro", f"Erro ao configurar inicialização automática: {str(e)}"))
    ...
    def add_pair(self):
        ...
        def confirm():
            ...
            if source and destination and self.validate_paths(source, destination):
                ...
                messagebox.showwarning("Entrada Inválida", "Por favor, selecione diretórios válidos.")
